...
improvement over NHC on a 5-mode Gaussian mixture. Direct measurement of
inter-mode transitions reveals the mechanism quantitatively: N=1 thermostats
(alpha=12, narrow-band) make zero mode hops in 2M force evaluations, while
N=3 (1/f, alpha=1.04) completes 466 transitions with integrated autocorrelation
time tau_int = 20,546 force evaluations—a 2.4x reduction relative to N=1.
An empirical scaling law tau_int ~ exp(-0.241 * alpha) governs mixing across
the mobile regime (N >= 3). Hormander bracket analysis confirms that
non-ergodicity in Nose-Hoover is dynamical (KAM), not geometric: all friction
functions achieve full bracket rank. These results establish bounded-friction,
multi-scale thermostats as a practical and theoretically grounded alternative
to NHC for canonical sampling.

**Keywords:** thermostat, canonical ensemble, ergodicity, KAM tori, 1/f noise,
Nose-Hoover, Liouville equation, generalized friction, mixing time, mode hopping
...
4. **1/f noise via multi-scale dynamics.** Coupling N thermostat variables
   ...
   GMM KL by 16x relative to NHC on a 5-mode Gaussian mixture, completing
   466 inter-mode transitions in 2M force evaluations versus zero for
   narrow-band (N=1) configurations.
...
### 3.3 Mode-Hop Evidence: The Smoking Gun

The most direct evidence for the 1/f mixing advantage comes from counting
inter-mode transitions directly. We ran 2M force evaluations on a 5-mode
Gaussian mixture and counted mode hops—transitions between the five Gaussian
components—as a function of N (number of thermostats).

**The most striking result: N=1 thermostats (narrow-band, alpha=12.1) make
zero inter-mode transitions in 2M force evaluations—the sampler is completely
trapped. N=3 (1/f, alpha=1.04) completes 466 transitions.**

Full tau_int table (tau_int measured in force evaluations):

| N  | alpha  | tau_int (force-evals) | mode hops / 2M evals |
|----|--------|----------------------|----------------------|
| 1  | 12.1   | 50,003               | 0 (STUCK)            |
| 2  | —      | 48,284               | 15                   |
| 3  | 1.04   | **20,546**           | 466                  |
| 5  | 1.95   | 24,619               | 458                  |
| 7  | 2.04   | 15,736               | 446                  |
...
N=3 is the minimum number of thermostats for ergodic mode exploration; it is
the transition point from trapped (N < 3) to mobile (N >= 3). The 6x KL
improvement reported in Section 3.2 corresponds concretely to a 2.4x reduction
in integrated autocorrelation time: tau_int(N=3) = 20,546 versus
tau_int(N=1) = 50,003 force evaluations.

Within the mobile regime (N >= 3), additional thermostats further reduce
tau_int with diminishing returns. The scaling follows an empirical law:

    tau_int ~ exp(-0.241 * alpha)    (excluding N=1 outlier)          (6)

This exponential dependence on spectral exponent alpha provides a quantitative
handle on thermostat design: each unit increase in alpha (toward Brownian noise)
reduces mixing time by a factor of exp(0.241) ~ 1.27. The law holds across the
mobile regime but breaks down sharply at the trapping transition (N=1 is an
outlier that lies far above the predicted value).
...
### 3.4 Barrier-Height Dependence

The mode-hop advantage of 1/f noise (N=3) over narrow-band (N=1) is
barrier-height dependent. We measured barrier crossings per 1000 force
evaluations as a function of barrier height lambda:

| lambda | N=1 (crossings/1k) | N=3 (crossings/1k) | N=5 (crossings/1k) | N3/N1 |
|--------|--------------------|--------------------|--------------------|----|
| 1 kT   | 1.51               | 1.46               | 1.53               | 0.96x  |
| 2 kT   | 0.66               | 0.88               | 0.79               | 1.34x  |
| 4 kT   | 0.18               | 0.16               | 0.18               | 0.88x  |
| 8 kT   | 0.00               | 0.00               | 0.01               | N/A    |

At moderate barriers (2 kT), N=3 (1/f) outperforms N=1 by 34%. At very low
barriers (1 kT) and very high barriers (>= 4 kT), differences are negligible or
reversed. Notably, at very high barriers (>= 4 kT), Brownian noise (N=5)
marginally outperforms 1/f—suggesting the optimal spectrum shifts with barrier
height. For the moderate-barrier multi-modal distributions that are most
practically relevant, 1/f (N=3) is the preferred configuration.
...
### 4.3 Efficiency in Force Evaluations

The quantitative mixing time for the champion configuration is
tau_int(N=3) = 20,546 force evaluations on the GMM benchmark. For reference,
NHC (N=1 equivalent, tau_int ~ 50,003) requires 2.4x more force evaluations
to achieve the same decorrelation.

Translating to effective sample size per force evaluation:

| Method       | ESS / force-eval | Relative efficiency |
|--------------|------------------|---------------------|
| NHC (M=3)    | ~0.003           | 1x (baseline)       |
| NHCTail (N=3)| ~0.010           | **3x**              |

The 3x improvement in ESS per force evaluation is the headline efficiency
number. It reflects both the reduction in tau_int and the absence of mode
trapping that wastes evaluations on redundant samples within a single mode.

### 4.4 Ergodicity on Harmonic Oscillator
...
4. **Multi-scale 1/f thermostats**: N=3 log-spaced thermostats achieve alpha=0.98,
   GMM KL = 0.054 (16x over NHC), 466 inter-mode transitions per 2M force
   evaluations (vs. zero for narrow-band), and tau_int = 20,546 force evaluations—
   a 2.4x reduction from tau_int = 50,003 for NHC.
...
### 6.3 The N=3 transition and optimality

The mode-hop data establish a precise statement about the role of N. N=3 is
the minimum number of thermostats for ergodic mode exploration: it is the
transition point from trapped (N < 3, zero mode hops over 2M force
evaluations) to mobile (N >= 3, hundreds of mode hops). The claim that
"1/f is optimal" should therefore be understood precisely:

- N=3 achieves 1/f noise (alpha ~ 1.04) and is the *entry point* to the
  mobile regime. It is not globally optimal in tau_int—larger N continue to
  reduce tau_int—but it is the most efficient entry point, providing the
  largest per-thermostat mixing gain.
- Within the mobile regime, adding more thermostats further reduces tau_int
  following tau_int ~ exp(-0.241 * alpha), but with diminishing returns.
- The optimality of N=3 / 1/f is therefore specific to moderate-barrier
  multi-modal distributions. At very high barriers (>= 4 kT), Brownian
  noise (N=5) marginally outperforms 1/f, suggesting the optimal spectrum
  shifts with barrier height (Section 3.4).
...
For **multi-modal or moderate-barrier systems**: use MultiScaleNHCTail with N=3
log-spaced Q values spanning 2 decades (e.g., [0.1, 0.7, 10.0]) and
chain_length=2. This configuration achieves the 1/f transition point and
ESS/force-eval ~ 0.010 (3x over NHC).

For **very high-barrier systems** (barriers >= 4 kT): consider N=5 (Brownian
noise, alpha ~ 2) which marginally outperforms 1/f at extreme barriers.
...
6. *Optimal friction PSD for a given target:* What is the optimal friction
   power spectral density for a given target distribution? Our results
   suggest 1/f for multi-modal distributions with moderate barriers, and
   Brownian (alpha ~ 2) at very high barriers, but a systematic theory of
   optimal spectral design—relating the barrier height distribution of the
   target to the optimal alpha—remains open.
